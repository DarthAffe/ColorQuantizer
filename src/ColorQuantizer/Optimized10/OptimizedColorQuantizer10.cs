...
{
    public static class OptimizedColorQuantizer10
    {
        public static SKColor[] Quantize(SKColor[] colors, int amount)
        {
        ...
        public static SKColor[] QuantizeSplit(SKColor[] colors, int splits)
        {
            ...
            Span<SKColor> colorSpan = colors.AsSpan();

            ColorCube[] cubeArray = new ColorCube[1 << splits];
            Span<ColorCube> cubes = cubeArray;
            cubes[0] = new ColorCube(colorSpan, 0, colorSpan.Length, SortTarget.None);

            for (int i = 0; i < splits; i++)
            {
                int currentCubeCount = 1 << i;
                ParallelHelper.For(0, currentCubeCount, new CubeCreator(colors, cubeArray, currentCubeCount));
            }

            SKColor[] result = new SKColor[cubes.Length];
            ParallelHelper.For(0, cubes.Length, new Averager(colors, cubeArray, result));

            return result;
        }

        internal readonly struct CubeCreator : IAction
        {
            private readonly SKColor[] _colors;
            private readonly ColorCube[] _cubeArray;
            private readonly int _currentSize;

            public CubeCreator(SKColor[] colors, ColorCube[] cubeArray, int currentSize)
            {
                this._colors = colors;
            ...
            {
                _cubeArray[i].Split(_colors.AsSpan(), out ColorCube a, out ColorCube b);
                _cubeArray[i] = a;
                _cubeArray[_currentSize + i] = b;
            }
        }

        internal readonly struct Averager : IAction
        {
            private readonly SKColor[] _colors;
            private readonly ColorCube[] _cubes;
            private readonly SKColor[] _result;

            public Averager(SKColor[] colors, ColorCube[] cubes, SKColor[] result)
            {
                this._colors = colors;
                this._cubes = cubes;
                this._result = result;
            ...
            public void Invoke(int i)
            {
                _result[i] = _cubes[i].GetAverageColor(_colors);
            }
        }
