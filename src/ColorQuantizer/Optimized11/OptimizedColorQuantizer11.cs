...
{
    public static class OptimizedColorQuantizer11
    {
        /// <summary>
        ...
        public unsafe static SKColor[] QuantizeSplit(in Span<SKColor> colors, int splits)
        {
            ...
            Span<uint> internalColors = MemoryMarshal.Cast<SKColor, uint>(colors);

            Span<ColorCube> cubes = new ColorCube[1 << splits];
            cubes[0] = new ColorCube(internalColors, 0, colors.Length, SortTarget.None);
            ...
            for (int i = 0; i < splits; i++)
            {
                ...
                {
                    currentCubes[j].Split(internalColors, out ColorCube a, out ColorCube b);
                    currentCubes[j] = a;
            ...
            for (int i = 0; i < cubes.Length; i++)
                result[i] = cubes[i].GetAverageColor(internalColors);
