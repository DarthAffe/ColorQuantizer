...
{
    internal readonly struct ColorRangesFor
    {
        public readonly byte RedRange;
        public readonly byte GreenRange;
        public readonly byte BlueRange;

        public ColorRangesFor(byte redRange, byte greenRange, byte blueRange)
        {
    ...
    internal readonly struct ColorCubeFor
    {
        ...
        public ColorCubeFor(in Span<SKColor> fullColorList, int from, int length, SortTarget preOrdered)
        {
            this._from = from;
            this._length = length;
            ...
            else if (colorRanges.GreenRange > colorRanges.BlueRange)
            {
                if (preOrdered != SortTarget.Green)
                    QuantizerSortFor.SortGreen(colors);
            ...
            else
            {
                if (preOrdered != SortTarget.Blue)
                    QuantizerSortFor.SortBlue(colors);
        ...
        private ColorRangesFor GetColorRanges(in ReadOnlySpan<SKColor> colors)
        {
            if (Vector.IsHardwareAccelerated && (colors.Length >= Vector<byte>.Count))
            {
                ...
                return new ColorRangesFor((byte)(redMax - redMin), (byte)(greenMax - greenMin), (byte)(blueMax - blueMin));
            }
            else
            {
                byte redMin = byte.MaxValue;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal void Split(in Span<SKColor> fullColorList, out ColorCubeFor a, out ColorCubeFor b)
        {
            Span<SKColor> colors = fullColorList.Slice(_from, _length);

            int median = colors.Length / 2;

            a = new ColorCubeFor(fullColorList, _from, median, _currentOrder);
            b = new ColorCubeFor(fullColorList, _from + median, colors.Length - median, _currentOrder);
        }
