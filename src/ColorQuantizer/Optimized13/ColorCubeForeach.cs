...
{
    internal readonly struct ColorRangesForeach
    {
        public readonly byte RedRange;
        public readonly byte GreenRange;
        public readonly byte BlueRange;

        public ColorRangesForeach(byte redRange, byte greenRange, byte blueRange)
        {
    ...
    internal readonly struct ColorCubeForeach
    {
        ...
        public ColorCubeForeach(in Span<SKColor> fullColorList, int from, int length, SortTarget preOrdered)
        {
            this._from = from;
            this._length = length;
            ...
            else if (colorRanges.GreenRange > colorRanges.BlueRange)
            {
                if (preOrdered != SortTarget.Green)
                    QuantizerSortForeach.SortGreen(colors);
            ...
            else
            {
                if (preOrdered != SortTarget.Blue)
                    QuantizerSortForeach.SortBlue(colors);
        ...
        private ColorRangesForeach GetColorRanges(in ReadOnlySpan<SKColor> colors)
        {
            if (Vector.IsHardwareAccelerated && (colors.Length >= Vector<byte>.Count))
            {
                ...
                return new ColorRangesForeach((byte)(redMax - redMin), (byte)(greenMax - greenMin), (byte)(blueMax - blueMin));
            }
            else
            {
                byte redMin = byte.MaxValue;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal void Split(in Span<SKColor> fullColorList, out ColorCubeForeach a, out ColorCubeForeach b)
        {
            Span<SKColor> colors = fullColorList.Slice(_from, _length);

            int median = colors.Length / 2;

            a = new ColorCubeForeach(fullColorList, _from, median, _currentOrder);
            b = new ColorCubeForeach(fullColorList, _from + median, colors.Length - median, _currentOrder);
        }
