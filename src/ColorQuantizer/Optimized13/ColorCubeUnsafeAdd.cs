...
{
    internal readonly struct ColorRangesUnsafeAdd
    {
        public readonly byte RedRange;
        public readonly byte GreenRange;
        public readonly byte BlueRange;

        public ColorRangesUnsafeAdd(byte redRange, byte greenRange, byte blueRange)
        {
    ...
    internal readonly struct ColorCubeUnsafeAdd
    {
        ...
        public ColorCubeUnsafeAdd(in Span<SKColor> fullColorList, int from, int length, SortTarget preOrdered)
        {
            this._from = from;
            this._length = length;
            ...
            else if (colorRanges.GreenRange > colorRanges.BlueRange)
            {
                if (preOrdered != SortTarget.Green)
                    QuantizerSortUnsafeAdd.SortGreen(colors);
            ...
            else
            {
                if (preOrdered != SortTarget.Blue)
                    QuantizerSortUnsafeAdd.SortBlue(colors);
        ...
        private ColorRangesUnsafeAdd GetColorRanges(in ReadOnlySpan<SKColor> colors)
        {
            if (Vector.IsHardwareAccelerated && (colors.Length >= Vector<byte>.Count))
            {
                ...
                foreach (Vector<byte> currentVector in colors[..vectorElements].Cast<SKColor, Vector<byte>>())
                {
                ...
                return new ColorRangesUnsafeAdd((byte)(redMax - redMin), (byte)(greenMax - greenMin), (byte)(blueMax - blueMin));
            }
            else
            {
                byte redMin = byte.MaxValue;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal void Split(in Span<SKColor> fullColorList, out ColorCubeUnsafeAdd a, out ColorCubeUnsafeAdd b)
        {
            Span<SKColor> colors = fullColorList.Slice(_from, _length);

            int median = colors.Length / 2;

            a = new ColorCubeUnsafeAdd(fullColorList, _from, median, _currentOrder);
            b = new ColorCubeUnsafeAdd(fullColorList, _from + median, colors.Length - median, _currentOrder);
        }
