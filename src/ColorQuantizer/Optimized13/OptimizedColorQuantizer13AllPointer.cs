...
{
    public static class OptimizedColorQuantizer13AllPointer
    {
        /// <summary>
        ...
        public static SKColor[] QuantizeSplit(in Span<SKColor> colors, int splits)
        {
            ...
            Span<ColorCubeAllPointer> cubes = new ColorCubeAllPointer[1 << splits];
            cubes[0] = new ColorCubeAllPointer(colors, 0, colors.Length, SortTarget.None);

            int currentIndex = 0;
            for (int i = 0; i < splits; i++)
            {
                int currentCubeCount = 1 << i;
                Span<ColorCubeAllPointer> currentCubes = cubes.Slice(0, currentCubeCount);
                for (int j = 0; j < currentCubes.Length; j++)
                {
                    currentCubes[j].Split(colors, out ColorCubeAllPointer a, out ColorCubeAllPointer b);
                    currentCubes[j] = a;
