...
{
    public static class OptimizedColorQuantizer13For
    {
        /// <summary>
        ...
        public static SKColor[] QuantizeSplit(in Span<SKColor> colors, int splits)
        {
            ...
            Span<ColorCubeFor> cubes = new ColorCubeFor[1 << splits];
            cubes[0] = new ColorCubeFor(colors, 0, colors.Length, SortTarget.None);

            int currentIndex = 0;
            for (int i = 0; i < splits; i++)
            {
                int currentCubeCount = 1 << i;
                Span<ColorCubeFor> currentCubes = cubes.Slice(0, currentCubeCount);
                for (int j = 0; j < currentCubes.Length; j++)
                {
                    currentCubes[j].Split(colors, out ColorCubeFor a, out ColorCubeFor b);
                    currentCubes[j] = a;
