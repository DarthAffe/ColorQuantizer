...
{
    public static class OptimizedColorQuantizer13Foreach
    {
        /// <summary>
        ...
        public static SKColor[] QuantizeSplit(in Span<SKColor> colors, int splits)
        {
            ...
            Span<ColorCubeForeach> cubes = new ColorCubeForeach[1 << splits];
            cubes[0] = new ColorCubeForeach(colors, 0, colors.Length, SortTarget.None);

            int currentIndex = 0;
            for (int i = 0; i < splits; i++)
            {
                int currentCubeCount = 1 << i;
                Span<ColorCubeForeach> currentCubes = cubes.Slice(0, currentCubeCount);
                for (int j = 0; j < currentCubes.Length; j++)
                {
                    currentCubes[j].Split(colors, out ColorCubeForeach a, out ColorCubeForeach b);
                    currentCubes[j] = a;
