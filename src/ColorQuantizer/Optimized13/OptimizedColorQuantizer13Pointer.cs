...
{
    public static class OptimizedColorQuantizer13Pointer
    {
        /// <summary>
        ...
        public static SKColor[] QuantizeSplit(in Span<SKColor> colors, int splits)
        {
            ...
            Span<ColorCubePointer> cubes = new ColorCubePointer[1 << splits];
            cubes[0] = new ColorCubePointer(colors, 0, colors.Length, SortTarget.None);

            int currentIndex = 0;
            for (int i = 0; i < splits; i++)
            {
                int currentCubeCount = 1 << i;
                Span<ColorCubePointer> currentCubes = cubes.Slice(0, currentCubeCount);
                for (int j = 0; j < currentCubes.Length; j++)
                {
                    currentCubes[j].Split(colors, out ColorCubePointer a, out ColorCubePointer b);
                    currentCubes[j] = a;
