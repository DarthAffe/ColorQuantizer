...
{
    public static class OptimizedColorQuantizer13UnsafeAdd
    {
        /// <summary>
        ...
        public static SKColor[] QuantizeSplit(in Span<SKColor> colors, int splits)
        {
            ...
            Span<ColorCubeUnsafeAdd> cubes = new ColorCubeUnsafeAdd[1 << splits];
            cubes[0] = new ColorCubeUnsafeAdd(colors, 0, colors.Length, SortTarget.None);

            int currentIndex = 0;
            for (int i = 0; i < splits; i++)
            {
                int currentCubeCount = 1 << i;
                Span<ColorCubeUnsafeAdd> currentCubes = cubes.Slice(0, currentCubeCount);
                for (int j = 0; j < currentCubes.Length; j++)
                {
                    currentCubes[j].Split(colors, out ColorCubeUnsafeAdd a, out ColorCubeUnsafeAdd b);
                    currentCubes[j] = a;
