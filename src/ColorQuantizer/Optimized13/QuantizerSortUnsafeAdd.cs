...
{
    internal static class QuantizerSortUnsafeAdd
    {
        #region Methods

        public static void SortRed(in Span<SKColor> colors)
        {
            ref SKColor colorsReference = ref MemoryMarshal.GetReference(colors);

            Span<int> counts = stackalloc int[256];
            for (int i = 0; i < colors.Length; i++)
            {
                SKColor color = Unsafe.Add(ref colorsReference, i);
                counts[color.Red]++;
            }
