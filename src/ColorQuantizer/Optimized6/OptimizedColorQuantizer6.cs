...
{
    public class OptimizedColorQuantizer6
    {
        public SKColor[] Quantize(in ReadOnlySpan<SKColor> colors, int amount)
        {
            if ((amount & (amount - 1)) != 0)
                throw new ArgumentException("Must be power of two", nameof(amount));

            Color[] internalColorArray = ArrayPool<Color>.Shared.Rent(colors.Length);
            Span<Color> internalColors = internalColorArray.AsSpan()[..colors.Length];
            for (int j = 0; j < colors.Length; j++)
            {
                SKColor color = colors[j];
                internalColors[j] = new Color(color.Red, color.Green, color.Blue);
            }

            Queue<ColorCube> cubes = new(amount);
            cubes.Enqueue(new ColorCube(internalColors, 0, colors.Length, SortTarget.None));

            while (cubes.Count < amount)
            {
                ColorCube cube = cubes.Dequeue();

                if (cube.TrySplit(internalColors, out ColorCube? a, out ColorCube? b))
                {
                    cubes.Enqueue(a);
                    cubes.Enqueue(b);
                }
            ...
            {
                Color color = colorCube.GetAverageColor(internalColors);
                result[i++] = new SKColor(color.Red, color.Green, color.Blue);
            }

            ArrayPool<Color>.Shared.Return(internalColorArray);
