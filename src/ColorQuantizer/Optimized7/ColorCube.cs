...
namespace ColorQuantizer.Optimized7
{
    ...
    {
        private static readonly int ELEMENTS_PER_VECTOR = Vector<byte>.Count / 3;
        private static readonly int BYTES_PER_VECTOR = ELEMENTS_PER_VECTOR * 3;
        ...
        private unsafe ColorRanges GetColorRanges(in ReadOnlySpan<Color> colors)
        {
            if (Vector.IsHardwareAccelerated && (colors.Length >= Vector<byte>.Count))
            {
                ...
                fixed (byte* colorPtr = &MemoryMarshal.GetReference(colorBytes))
                {
                    ...
                    for (int i = 0; i < BYTES_PER_VECTOR; i += 3)
                    {
