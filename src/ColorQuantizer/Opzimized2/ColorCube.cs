...
using ColorQuantizer.Sorting;
using SkiaSharp;

namespace ColorQuantizer.Opzimized2
{
    ...
    internal class ColorCube
    {
        ...
        private SKColor[] _sortArray;
        private SortTarget _currentOrder = SortTarget.None;

        public ColorCube(in Span<SKColor> fullColorList, int from, int length, in SKColor[] sortArray, SortTarget preOrdered)
        {
            this._from = from;
            this._length = length;
        ...
        private void OrderColors(in Span<SKColor> colors, SortTarget preOrdered)
        {
            ...
            if ((colorRanges.RedRange > colorRanges.GreenRange) && (colorRanges.RedRange > colorRanges.BlueRange))
            {
                if (preOrdered != SortTarget.Red)
                    QuantizerTimsortRed.Sort(colors, _sortArray);
            ...
            else if (colorRanges.GreenRange > colorRanges.BlueRange)
            {
                if (preOrdered != SortTarget.Green)
                    QuantizerTimsortGreen.Sort(colors, _sortArray);
            ...
            else
            {
                if (preOrdered != SortTarget.Blue)
                    QuantizerTimsortBlue.Sort(colors, _sortArray);
        ...
        internal bool TrySplit(in Span<SKColor> fullColorList, [NotNullWhen(returnValue: true)] out ColorCube? a, [NotNullWhen(returnValue: true)] out ColorCube? b)
        {
            ...
            a = new ColorCube(fullColorList, _from, median, _sortArray, _currentOrder);
            b = new ColorCube(fullColorList, _from + median, colors.Length - median, _sortArray, _currentOrder);
